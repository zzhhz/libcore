...
public class HActivityStack
{
    private static HActivityStack sInstance;
    ...
    private HActivityStack()
    {
    }

    public static HActivityStack getInstance()
    {
        if (sInstance == null)
        {
            synchronized (HActivityStack.class)
            {
                if (sInstance == null)
                    sInstance = new HActivityStack();
            }
        }
        return sInstance;
    }
    ...
    private final Application.ActivityLifecycleCallbacks mActivityLifecycleCallbacks = new Application.ActivityLifecycleCallbacks()
    {
        ...
        public void onActivityResumed(Activity activity)
        {
            ...
            {
                if (mIsDebug)
                    Log.e(HActivityStack.class.getSimpleName(), "start order activity " + activity + " old index " + index);

                removeActivity(activity);
                addActivity(activity);

                if (mIsDebug)
                    Log.e(HActivityStack.class.getSimpleName(), "end order activity " + activity + " new index " + mActivityHolder.indexOf(activity));
            }
        }
    ...
    private void addActivity(Activity activity)
    {
        ...
        if (mIsDebug)
        {
            Log.i(HActivityStack.class.getSimpleName(), "+++++ " + activity + " " + mActivityHolder.size()
                    + "\r\n" + getCurrentStack());
        }
    }
    ...
    private void removeActivity(Activity activity)
    {
        if (mActivityHolder.remove(activity))
        {
            if (mIsDebug)
            {
                Log.e(HActivityStack.class.getSimpleName(), "----- " + activity + " " + mActivityHolder.size()
                        + "\r\n" + getCurrentStack());
            }
        }
    }
